...
fn cannot_vote_two_times(mut session: Session) -> TestResult {
    ...
    let _first_vote = session.call::<_, VotingError>("vote_for", NO_ARGS, NO_ENDOWMENT)?;

    let error =
        session.call_and_expect_error::<_, VotingError>("vote_against", NO_ARGS, NO_ENDOWMENT)?;
...
fn voting_works(mut session: Session) -> TestResult {
    ...
    session.call::<_, VotingError>("vote_for", NO_ARGS, NO_ENDOWMENT)??;
